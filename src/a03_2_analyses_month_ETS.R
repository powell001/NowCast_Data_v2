...
forecast_dir <- "output_mo/forecasts/"

# remove files
do.call(file.remove, list(list.files("output_mo/analyses", full.names = TRUE)))
...
percent_movers <- percent_df[percent_df$ObservationDate %like% format(Sys.Date(), "%b %Y"), ]
